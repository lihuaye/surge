...
配置为手动导入（不含托管自动更新头）。本仓库更新后，需在 Surge 里重新下载配置才会生效——避免把你的本地改动（节点订阅、MITM 证书等）被自动同步覆盖。
...
fork 后需要改这些 `lihuaye/surge` 引用，全部换成你的 `用户名/仓库名`：
...
之后按需调整：`FINAL` 兜底指向 `Proxy` 组（未匹配流量经多地区自动选点承接，避免压在单一节点上）；建议把自己机场/自建节点的服务器域名填进 `[Host]` 的「订阅节点域名」模板（指向阿里 DoH），否则代理全挂时节点域名解析会陷入死锁、网络无法自愈；两份配置自 `[Proxy Group]` 起需保持逐行一致，改分流规则请两边同步。
